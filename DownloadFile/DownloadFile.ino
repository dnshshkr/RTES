...
void loop()
{
  ...
  if (Firebase.ready() && !taskCompleted)
  {
    ...
    if (!Firebase.Storage.download(&fbdo, STORAGE_BUCKET_ID /* Firebase Storage bucket id */, "file1.txt" /* path of remote file stored in the bucket */, "/file1.txt" /* path to local file */, mem_storage_type_flash /* memory storage type, mem_storage_type_flash and mem_storage_type_sd */, fcsDownloadCallback /* callback function */))
      Serial.println(fbdo.errorReason());
    File file = SPIFFS.open("/file1.txt", FILE_READ);
