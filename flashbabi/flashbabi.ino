...
#define DATABASE_URL "rtes-378707-rtdb.asia-southeast1.firebasedatabse.app" //<databaseName>.firebaseio.com or <databaseName>.<region>.firebasedatabase.app
...
void setup()
{
  ...
  File file1 = SPIFFS.open("/file1.txt", FILE_WRITE);
  if (!file1.print("rindu awani"))
    Serial.println("failed to write file1");
  else
    Serial.println("file1 write successful");
  file1.close();

  File file2 = SPIFFS.open("/file2.txt", FILE_WRITE);
  if (!file2.print("awani pls comeback :("))
    Serial.println("failed to write file2");
  else
    Serial.println("file1 write successful");
  file2.close();
}
...
void loop()
{
  ...
  if (Firebase.ready() && !taskCompleted)
  {
    ...
    for (int i = 1; i <= 2; i++)
    {
      ...
      File fileTemp = SPIFFS.open(fileName, FILE_READ);
      if (!fileTemp)
