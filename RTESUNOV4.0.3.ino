...
float flowRateBias = 1.14;              //flowrate mililliter per pulse //CMD
float solShotBias = 1.4;                //solenoid mililliter per shot //CMD
unsigned long manualTimePeriod = 5000;  //set time for manual mode
volatile int pulsePeriodTime = 1000;             //set period time for pulse sensor capturing data (millisecond) 10=280
int currentSensorType = 1;              //'0'=ACS713 '1'=ACS712
int noiseRejection = 150;               //in ms //CMD
uint8_t solenoidOnTime = 1;
...
void setup()
{
  ...
  attachInterrupt(digitalPinToInterrupt(pulseFlowrate), pulseMeasure, FALLING);
